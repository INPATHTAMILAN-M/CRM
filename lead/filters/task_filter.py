    ...
    def filter_team(self, queryset, name, value):
        ...
        if is_bdm:
            ...
            if str(value).lower() == "true":
                ...
                return queryset.filter(
                    Q(contact__lead__assigned_to__in=team_member_ids) |
                    Q(contact__lead__created_by__in=team_member_ids)
                )

            # When ?team=false → only show own records
            return queryset.filter(
                Q(contact__lead__assigned_to=user.id) |
                Q(contact__lead__created_by=user.id)
            )
